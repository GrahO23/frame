...
app.get("/tube", function(req, res) {
  console.log("/tube/");
  unirest
    .get("http://cloud.tfl.gov.uk/TrackerNet/LineStatus")
...
function toggleLight(callback) {
  if (ring) {
    ring.devices((e, devices) => {
      ...
      if (
        devices &&
        devices.hasOwnProperty("stickup_cams") &&
...
function listenForVolumeChanges(lastVolume) {
  ...
  sonosClient.volume().then(function(volume) {
    if (
      lastVolume &&
      (lastVolume.volume !== volume.volume || lastVolume.muted !== volume.muted)
    ) {
      // console.log('vol change: ' + volume );
      console.log(
        "Volume Changed: " +
          JSON.stringify(lastVolume) +
          "new vol: " +
